...
import { BookOpen, GraduationCap, UserCheck, ArrowLeft, LogIn } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = (role) => { window.location.href = `${apiBase}/auth/google?role=${role}`; };
      ...
      <div className="hidden lg:flex w-1/2 bg-brand-950 flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative">
          <blockquote className="text-4xl font-display font-bold text-white leading-tight mb-6">
            "{t('Every student deserves a mentor who believes in them.')}"
          </blockquote>
          <p className="text-brand-300 text-sm">{t('Connecting students with qualified mentors and senior peers.')}</p>
        </div>
        <div className="relative flex gap-6">
          {[['500+', t('Students')], ['120+', t('Mentors')], ['8', t('Districts')]].map(([n, l]) => (
            <div key={l}>
          ...
          {!selectedRole ? (
            <div className="space-y-8" style={{ animation: 'fadeUp 0.5s ease forwards' }}>
              <div className="text-center lg:text-left">
                <h1 className="font-display font-bold text-3xl text-surface-900 mb-2">{t('Welcome back')}</h1>
                <p className="text-surface-500">{t('Please select your account type to continue.')}</p>
              </div>
              ...
              <div className="grid gap-4">
                ...
                >
                  ...
                  <div>
                    <h3 className="font-bold text-surface-900 text-lg">{t('Student Login')}</h3>
                    <p className="text-sm text-surface-500">{t('Access your courses and mentors')}</p>
                  </div>
                ...
                >
                  ...
                  <div>
                    <h3 className="font-bold text-surface-900 text-lg">{t('Mentor Login')}</h3>
                    <p className="text-sm text-surface-500">{t('Help students and manage sessions')}</p>
                  </div>
                  <div className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity">
                    <LogIn size={20} className="text-brand-600" />
                  </div>
              ...
              <div className="pt-6 border-t border-surface-100 text-center">
                <p className="text-sm text-surface-500 mb-4">{t('New to ShikshaSetu?')}</p>
                <div className="flex gap-3">
                  <Link to="/register/student" className="flex-1 btn-secondary justify-center text-sm py-2.5">{t('Join as Student')}</Link>
                  <Link to="/register/volunteer" className="flex-1 btn-secondary justify-center text-sm py-2.5">{t('Join as Mentor')}</Link>
                </div>
              </div>
            </div>
          ) : (
            <div style={{ animation: 'fadeUp 0.4s ease forwards' }}>
              ...
              <h1 className="font-display font-bold text-3xl text-surface-900 mb-2">
                {selectedRole === 'student' ? t('Student Portal') : t('Mentor Portal')}
              </h1>
              <p className="text-surface-500 mb-8">{t('Sign in to your account with your credentials.')}</p>

              <div className="space-y-6">
                <div className="bg-brand-50 border border-brand-100 p-6 rounded-2xl">
                  {selectedRole === 'volunteer' && (
                    <p className="text-sm text-brand-700 mb-6 text-center">{t('To ensure quality and security, mentors must use their verified Google accounts.')}</p>
                  )}
                  {selectedRole === 'student' && (
                    <p className="text-sm text-brand-700 mb-6 text-center">{t('Students can securely access their learning dashboard using their Google account.')}</p>
                  )}
                  <button
                    type="button"
